...
void scanimate_modify_raster(double& sx, double& sy, double& dot_radius, double& signal, const unsigned int field,
	const unsigned long long fieldno, const double frame_t) {
	// modify here
...
void composite_layer(
	...
	for (; y < srcframe->height; y += ystep) {
		for (x = 0; x < (srcframe->width << PRECISION); x++) {
			...
			frame_t = ((double)((y * srcframe->width * (1 << PRECISION)) + x)) /
					  (srcframe->width * srcframe->height * (1 << PRECISION));
...
int main(int argc, char** argv) {
	...
	/* run all inputs and render to output, until done */
	{
		...
		do {
			...
			while (current < upto) {
				memset(output_avstream_video_frame->data[0], 0,
