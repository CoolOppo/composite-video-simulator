...
public:
	...
	~ConvolutionMap() { freemap(); }
	bool allocmap(const size_t len) {
		if (map != NULL && len == length) return true;
...
public:
	...
	double calc(double s) {
		...
		for (i = 0; i < length; i++) r += map[i] * multiply[i];
...
double		   lr_delay =
	2;  // part of head tilt, as a consequence of storing stereo left + right on separate halves of the tape
...
void composite_audio_process(
	...
	for (unsigned int s = 0; s < samples; s++, audio += output_audio_channels) {
		...
		if (mono_downmix) audio[0] = audio[1] = (audio[0] + audio[1]) / 2;
			...
			if (!strcmp(a, "h") || !strcmp(a, "help")) {
				...
				switch (preset) {
					...
					case 4: /* good recording, good deck */
						...
						head_tilt			  = 1.1;
						break;
					default: fprintf(stderr, "Unknown preset\n"); return 1;
				};
			} else {
	...
	if (emulating_preemphasis) {
		for (unsigned int i = 0; i < output_audio_channels; i++) {
			audio_linear_preemphasis_pre[i].setFilter(
				output_audio_rate, 4000 /*FIXME: Guess! Also let user set this.*/);
		}
	}
	if (emulating_deemphasis) {
		for (unsigned int i = 0; i < output_audio_channels; i++) {
