...
static void help(const char* arg0) {
	...
	fprintf(stderr, "\n");
	fprintf(
		stderr, "Note: Video is taken from first input file, and colormap taken from mid scanline of second video.\n");
}
...
void composite_layer(AVFrame* dstframe, AVFrame* srcframe, InputFile& inputfile) {
	...
	for (y = 0; y < dstframe->height; y++) {
		...
		for (x = 0; x < dstframe->width; x++, dscan++, sscan++) { *dscan = *sscan; }
	}
}
...
double gamma_dec(double x) { return pow(x, gamma_correction); }
...
double gamma_enc(double x) { return pow(x, 1.0 / gamma_correction); }
...
void gamma16_do_init(void) {
	...
	for (unsigned int i = 0; i < 256; i++) gamma_dec16_table[i] = (unsigned long)(gamma_dec(i / 255.0) * 8192);

	for (unsigned int i = 0; i <= 8192; i++) gamma_enc16_table[i] = (unsigned long)(gamma_enc(i / 8192.0) * 255);
}
...
int main(int argc, char** argv) {
	...
	if (output_avstream_video_resampler == NULL) {
		output_avstream_video_resampler = sws_getContext(
			// source
			output_avstream_video_frame->width, output_avstream_video_frame->height,
	...
	/* run all inputs and render to output, until done */
	{
		...
		memset(output_avstream_video_frame->data[0], 0x00,
			output_avstream_video_frame->linesize[0] * output_avstream_video_frame->height);
		...
		for (std::vector<InputFile>::iterator ifi = input_files.begin(); ifi != input_files.end(); ifi++) {
			...
			while (!DIE) {
				...
				if (input_file.eof && (input_file.video_frame_to_output_f() < -1000 /*AV_NOPTS_VALUE*/ ||
				...
				if (frames.size() > 1) {
					if (framealt > 1) {
						for (size_t i = (size_t)((unsigned long long)current % (unsigned long long)framealt);
							 (i + (size_t)framealt) < frames.size(); i += (size_t)framealt) {
							double bt = frame_t[i];
							double et = frame_t[i + framealt];

							if (i != 0) {
								if ((et + 2.0) < current) { cutoff = i - (i % framealt); }
							}

							if (bt < current) bt = current;
							if (bt > (current + (fullframealt ? framealt : 1)))
								bt = (current + (fullframealt ? framealt : 1));

							if (et < current) et = current;
						...
						for (size_t i = 0; (i + 1ul) < frames.size(); i++) {
							...
							if (i != 0) {
								if ((et + 2.0) < current) { cutoff = i; }
							}

							if (bt < current) bt = current;
							if (bt > (current + 1ll)) bt = (current + 1ll);

							if (et < current) et = current;
							if (et > (current + 1ll)) et = (current + 1ll);

							assert(bt <= et);

							if (bt < et) weights.push_back(pair<size_t, double>(i, et - bt));
						}
					}
				}

				if (weights.size() == 0 && frames.size() > cutoff) weights.push_back(pair<size_t, double>(cutoff, 1.0));
				...
				if (squelch_frameblend_near_match) {
					if (weights.size() == 2 || weights.size() == 3) {
						...
						sq = fabs((et - bt) - 1.0) /
							 0.01; /* start squelching if less than 1% difference between source & dest rate */
						if (sq < 1.0) {
							sq = pow(sq, 2.0);
							...
							if (weights.size() > 2) weights[2].second = 0.0;
						}
					}
				}
				...
				if (gamma_correction > 1) {
					for (unsigned int y = 0; y < output_height; y++) {
						unsigned char* outframe = (unsigned char*)(output_avstream_video_frame->data[0] +
																   (y * (output_avstream_video_frame->linesize[0])));
						for (unsigned int x = 0; x < output_width; x++) {
							...
							for (size_t wi = 0; wi < weight16.size(); wi++) {
								...
								unsigned char* inframe =
									((unsigned char*)frames[fi] +
										(y * (input_file.input_avstream_video_frame_rgb->linesize[0]))) +
									(x * 4u);
				...
				if (sws_scale(output_avstream_video_resampler,
						// source
						output_avstream_video_frame->data, output_avstream_video_frame->linesize, 0,
