...
void composite_layer(AVFrame* dstframe, AVFrame* srcframe, InputFile& inputfile) {
	...
	for (y = 0; y < dstframe->height; y++) {
		...
		for (x = 0; x < dstframe->width; x++, dscan++, sscan++) {
			...
			// run the source file through the composite video simulator first and it will look like 1980's green-screen
			// compositing.
			if (xdivc == 0) {
				dR = (*sscan >> 16UL) & 0xFF;
				dR -= (inputfile.color >> 16UL) & 0xFF;
				dG = (*sscan >> 8UL) & 0xFF;
				dG -= (inputfile.color >> 8UL) & 0xFF;
				dB = (*sscan >> 0UL) & 0xFF;
				dB -= (inputfile.color >> 0UL) & 0xFF;
			...
			while (current < upto) {
				for (std::vector<InputFile>::iterator i = input_files.begin(); i != input_files.end(); i++) {
					...
					composite_layer(output_avstream_video_frame[output_avstream_video_frame_index],
						(*i).input_avstream_video_frame_rgb, *i);
				}
